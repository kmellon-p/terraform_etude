...
3. 빌드서버 Tailscale로 보안 설정하기 _(인프라 외 작업, 라즈베리파이 OS에서 직접 설정)_
4. 빌드서버에 GitHub self-hosted runner 등록 _(라즈베리파이에서 직접 설정)_
...
3. `docker buildx` 로 `linux/amd64` 이미지 빌드 _(빌드서버는 arm64지만 EC2는 x86이라 크로스 빌드)_
